...
class BoardControllerTest {
    ...
    void createBoard_NoToken_Test() throws Exception {
        ...
        Mockito.when(jwtUtils.getTokenFromRequest(Mockito.any(HttpServletRequest.class))).thenReturn(null);
    ...
    void deleteBoard_NotFound_Test() throws Exception {
        ...
        Mockito.when(jwtUtils.getTokenFromRequest(Mockito.any(HttpServletRequest.class))).thenReturn(invalidToken);
        Mockito.doThrow(new ApplicationException("존재하지 않는 게시판입니다", HttpStatus.NOT_FOUND))
            .when(boardService).deleteBoard(Mockito.any(HttpServletRequest.class), Mockito.anyLong());

        ResultActions resultActions = mockMvc.perform(delete("/api/boards/1").with(csrf()));

        resultActions.andExpect(status().isNotFound())
            .andExpect(content().string("존재하지 않는 게시판입니다"));
    }
